...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  final IDetailProductRepository _productRepository;
  final IBasketRepository _basketRepository;
  ProductBloc(this._basketRepository, this._productRepository)
    ...
    on<ProductInitializeEvent>((event, emit) async {
      ...
      var productCategory =
          await _productRepository.getProductCategories(event.categoryId);
      var productName =
          await _productRepository.getProductName(event.productId);
      var productProperty =
          await _productRepository.getProductProperty(event.productId);

      emit(ProductDetailResponseState(productImages, productVariant,
          productCategory, productName, productProperty));
    });
